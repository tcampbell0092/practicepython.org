def tic_tac():
    # initialize the 3x3 game board
    board = [(['.'] * 3) for i in range(3)]

    # variables for input and turn count
    row_col = [0]
    turn = 1

    # define input parameters
    def input_valid(values):
        # input has only two values
        if len(values) != 2:
            print("Input must be two numbers in format row,col e.g.  1,2 ")
            return 0
        # input is a number between 1 and 3 (inclusive)
        try:
            if (1 <= int(values[0]) <= 3) and (1 <= int(values[1]) <= 3):
                # if 0 is same as 1, write as taken position
                if board[int(values[0]) - 1][int(values[1]) - 1] != '.':
                    print("Position on board already taken.")
                    return 0
                return 1
            else:
                print("Input values must be numbers between 1 and 3 (inclusive)")
                return 0
        except ValueError:
            print("Input values must be numbers between 1 and 3 (inclusive)")
            return 0

    # draw the board
    def draw_board(values, player):
        # changes the value to X (player 1) or O (player 2)
        board[int(values[0]) - 1][int(values[1]) - 1] = player

        # print the gameboard
        for row in board:
            print(row)

    # calculate if game is over (no more '.' or has winner)
    def game_over():
        search = '.'

        # check win by row
        for i in range(3):
            if len(set(board[i])) == 1:
                if board[i][1] == '.':
                    continue
                elif board[i][1] == 'X':
                    print("Game over - Player 1 wins horizontally.")
                # elif gameboard[i][1] == 'O':
                else:
                    print("Game over - Player 2 wins horizontally.")
                return 1

        # check win by column
        for i in range(3):
            if board[0][i] == board[1][i] == board[2][i]:
                if board[0][i] == '.':
                    continue
                elif board[0][i] == 'X':
                    print("Game over - Player 1 wins vertically.")
                else:
                    print("Game over - Player 2 wins vertically.")
                return 1

        # check win by diagonal
        if (board[0][0] == board[1][1] == board[2][2]) or (
                board[0][2] == board[1][1] == board[2][0]):
            if board[1][1] == 'X':
                print("Game over - Player 1 wins diagonally.")
            elif board[1][1] == 'O':
                print("Game over - Player 2 wins diagonally.")
            else:
                return 0
            return 1

        # check board is full
        for sublist in board:
            if search in sublist:
                return 0

        print("Game over - the board is filled")
        return 1

    # main function that runs the game while board is not full
    while not game_over():
        piece = '.'

        # Player input - checks for input correctness
        while not input_valid(row_col):
            player = turn % 2
            if player == 0:
                player = 2
                piece = 'O'
            else:
                piece = 'X'
            p1 = input('Player ' + str(player) + ' input: ')
            row_col = p1.split(",")

        draw_board(row_col, piece)

        row_col = [0]
        turn += 1

        # asks the players if they would like to start a new game
        if game_over():
            restart = input("Would you like to play again? (y/n): ")
            if restart == 'y':
                tic_tac()
            else:
                print("Thanks for playing!")
                break


tic_tac()
